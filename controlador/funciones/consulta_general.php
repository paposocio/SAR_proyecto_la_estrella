<?php
include"../../modelo/modelo_1.php";
include"../sesion.php";
$objeto=new usuario ();
$retorno=$objeto->consultar($_SESSION);
?>
<html>
    <head>
        <link rel="icon" type="image/png" href="https://th.bing.com/th/id/R.0d12668f87faed591dbd730d692113b2?rik=hPZ2D9T%2bBiAP%2fg&riu=http%3a%2f%2fwww.chnafricaica.com%2fupload%2f2020%2f05%2f19%2fc4c64bff72d12c48348922d1115788bb.png&ehk=s6O%2fbTioXXzgUL6roZabIbd3OiXXTuw4ANWH%2b1468vU%3d&risl=&pid=ImgRaw&r=0" sizes="64x64">
        <meta charset="UTF-8">
        
        <!--Metainformacion necesaria-->
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>

        <!-- Bootstrap CSS -->
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/css/bootstrap.min.css">
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"></script>
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/js/bootstrap.min.js"></script>

        <!-- Datatables -->
        <link rel="stylesheet" href="https://cdn.datatables.net/v/bs4-4.1.1/dt-1.10.18/datatables.min.css">
        <script src="https://cdn.datatables.net/v/bs4-4.1.1/dt-1.10.18/datatables.min.js"></script>
        <style>
            #mydatatable tfoot input{
                width: 100% !important;
            }
            #mydatatable tfoot {
                display: table-header-group !important;
            }
            </style>
    </head>
    <body class="container-fluid p-5">
    <h1 style="text-align:center;font-size:larger">Usuarios</h1>
    <a href="../../vista/Interfaz_de_administrador.php">
            <center>
            <img style='width: 30px;'src='https://upload.wikimedia.org/wikipedia/commons/e/e6/Home_icon_black.png'>
            </center>
        </a>
    <a style='font-size: 20px;' href="pdf_usuarios.php">Exportar PDF
            <img style='width: 25px;'src='https://www.clubviana.org/wp-content/uploads/2020/07/icono-PDF.png'>
        </a>
        <div class="table-responsive" id="mydatatable-container">
            <table class="records_list table table-striped table-bordered table-hover" id="mydatatable">
            <caption>Usuarios</caption>
                <thead>
                    <tr>
                        <th>ID</th> 
                        <th>Nombre</th>
                        <th>Nombre usuario</th>
                        <th>Direccion</th>
                        <th>Estado</th>
                        <th>Tipo documento</th>
                        <th>Numero documento</th>
                        <th>Telefono</th>
                        <th>Contraseña</th>
                        <th>Titulo universitario</th>
                        <th>Genero</th>
                        <th>Rol usuario</th>
                        <th>Fecha de nacimiento</th>
                        <th>Correo electronico</th>
                        <th>Casa residente</th>
                        <th>Porte de armas</th>
                        <th>Años de experiencia</th>
                        <th>Acciones</th>
                        <!--17-->
                    </tr>
                </thead>
                <tfoot>
                    <tr>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                    </tr>
                </tfoot>
                <tbody>
                <?php
                    foreach ($retorno as $fila)
                    {
                        echo "<tr>";
        
                        foreach ($fila as $columna)
                        {
                            if (empty($columna))
                            {
                                echo "<td style='text-align:center;'>N/A</td>";    
                            }
                            else if ($columna == 'activo')
                            {  
                                echo "<td style='color:#648f24;'>
                                <img style='width: 10px;' src='https://www.pngkey.com/png/full/968-9682806_green-circle-png-fluorescent-yellow-circle.png'> 
                                $columna</td>";
                                $estado=1;
                            }
                            else if ($columna == 'inactivo')
                            {  
                                echo "<td style='color:#AEAEAE;font-size:15px;'>
                                <img style='width: 10px;' src='https://images.squarespace-cdn.com/content/v1/543a8586e4b06f81e20a529b/1509707246026-0TN7O0MYIQZRVXVDEW0Q/32012-L_gris_moyen_Light_Base_df9c44d3-d2ba-450a-9aac-991407158fca.png?format=1500w'> 
                                $columna</td>";
                                $estado=2;
                            }
                            else 
                            {
                                echo "<td>$columna</td>";
                            }    
                        }
                        if ($fila['rol_usuario']=='administrador')
                        {
                            echo "<td style='text-align:center;'>N/A</td>";
                            echo "</tr>";
                        }

                        else
                         {
                        echo    //1=actualizacion
                                //2=inactivacion
                                //3=activacion
                                "<td>
                                <a href='actualizar.php?
                                a=".(INT)$fila ["id_usuario"]." & 
                                b=".urlencode($fila ["nombre"])." & 
                                c=".urlencode($fila ["nombre_u"])." & 
                                d=".urlencode($fila ["direccion"])." & 
                                e=".(int)($estado)." & 
                                f=".urlencode($fila ["tip_doc"])." & 
                                g=".(INT)$fila ["num_doc"]." & 
                                h=".(INT)$fila ["telefono"]." & 
                                i=".urlencode($fila ["contrasena"])." & 
                                j=".urlencode($fila ["titulo_universitario"])." & 
                                k=".urlencode($fila ["genero"])." & 
                                l=".urlencode($fila ["rol_usuario"])." & 
                                m=".urlencode($fila ["fecha_nac"])." & 
                                n=".urlencode($fila ["correo"])." & 
                                o=".(INT)$fila ["casa_residente"]." & 
                                p=".urlencode($fila ["porte_armas"])." & 
                                q=".(INT)$fila ["anosexperiencia"]."&'>
                                <img style='width: 25px;'src='https://cdn.pixabay.com/photo/2016/03/31/18/32/edit-1294441_1280.png'>
                                </a>

                                <a href='acti_inacti.php?
                                a=".(INT)$fila ["id_usuario"]." & 
                                b=".urlencode($fila ["nombre"])." & 
                                c=".urlencode($fila ["nombre_u"])." & 
                                d=".urlencode($fila ["direccion"])." & 
                                e=".(2)." & 
                                f=".urlencode($fila ["tip_doc"])." & 
                                g=".(INT)$fila ["num_doc"]." & 
                                h=".(INT)$fila ["telefono"]." & 
                                i=".urlencode($fila ["contrasena"])." & 
                                j=".urlencode($fila ["titulo_universitario"])." & 
                                k=".urlencode($fila ["genero"])." & 
                                l=".urlencode($fila ["rol_usuario"])." & 
                                m=".urlencode($fila ["fecha_nac"])." & 
                                n=".urlencode($fila ["correo"])." & 
                                o=".(INT)$fila ["casa_residente"]." & 
                                p=".urlencode($fila ["porte_armas"])." & 
                                q=".(INT)$fila ["anosexperiencia"]."&'>
                                <img style='width: 25px;'src='https://www.freeiconspng.com/thumbs/turn-off-icon-png/red-turn-off-png-0.png'>
                                </a>

                                <a href='acti_inacti.php?                                
                                a=".(INT)$fila ["id_usuario"]." & 
                                b=".urlencode($fila ["nombre"])." & 
                                c=".urlencode($fila ["nombre_u"])." & 
                                d=".urlencode($fila ["direccion"])." & 
                                e=".(1)." & 
                                f=".urlencode($fila ["tip_doc"])." & 
                                g=".(INT)$fila ["num_doc"]." & 
                                h=".(INT)$fila ["telefono"]." & 
                                i=".urlencode($fila ["contrasena"])." & 
                                j=".urlencode($fila ["titulo_universitario"])." & 
                                k=".urlencode($fila ["genero"])." & 
                                l=".urlencode($fila ["rol_usuario"])." & 
                                m=".urlencode($fila ["fecha_nac"])." & 
                                n=".urlencode($fila ["correo"])." & 
                                o=".(INT)$fila ["casa_residente"]." & 
                                p=".urlencode($fila ["porte_armas"])." & 
                                q=".(INT)$fila ["anosexperiencia"]."&'>
                                <img style='width: 25px;'src='https://cdn.pixabay.com/photo/2012/04/11/18/29/button-29286_640.png'>
                                </a>

                                </td>" ;
                                echo "</tr>";
                         }


                    }
                    ?>
                </tbody>
            </table>
        </div>

            </body>
            </html>
            <script type="text/javascript">
                
                $(document).ready(function() {
                    $('#mydatatable tfoot th').each( function () {
                        var title = $(this).text();
                        $(this).html( '<input type="text" placeholder="Filtrar.." />' );
                    } );
                
                    var table = $('#mydatatable').DataTable({
                        "dom": 'B<"float-left"i><"float-right"f>t<"float-left"l><"float-right"p><"clearfix">',
                        "responsive": false,
                        "language": {
                            "url": "https://cdn.datatables.net/plug-ins/1.10.19/i18n/Spanish.json"
                        },
                        "order": [[ 0, "desc" ]],
                        "initComplete": function () {
                            this.api().columns().every( function () {
                                var that = this;
                
                                $( 'input', this.footer() ).on( 'keyup change', function () {
                                    if ( that.search() !== this.value ) {
                                        that
                                            .search( this.value )
                                            .draw();
                                        }
                                });
                            })
                        }
                    });
                });
                </script>   
